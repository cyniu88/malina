...
{
  _type = type & 0x0f;
  if (type & PRESSED_STATE_HIGH)
    _stateForPressed = true;
  _description = desc;
};
...
bool lkankowski::Button::getRelayState(bool relayState)
{
  ...
  bool result;
  if ((_type == MONO_STABLE) || (_type == BI_STABLE))
  { // toggle relay
    result = !relayState;
  }
...
int lkankowski::Button::getEvent(bool isPinChanged, int pinState)
{
  ...
  bool hasLongClick = _longclickRelayNum != -1; //cyniu  tu usunac
  bool hasDoubleClick = _doubleclickRelayNum != -1;

  unsigned long now = millis();

  if (_eventState == BTN_STATE_INITIAL)
  { // waiting for change
    if (isPinChanged)
    {
  ...
  else if (_eventState == BTN_STATE_1ST_CHANGE_BI)
  ...
  else if (_eventState == BTN_STATE_1ST_PRESS)
  { // waiting for 1st release
    ...
    { // still pressed
      if ((!hasDoubleClick) && (!hasLongClick) && (pinState == _monoStableTrigger))
      { // no long/double-click action, do click (old behavior)
  ...
  else if (_eventState == BTN_STATE_2ND_PRESS)
  ...
  else if (_eventState == BTN_STATE_RELEASE_WAIT)
  { // waiting for release after long press
    if (!activeLevel)
    { // released
      _eventState = BTN_STATE_INITIAL;
    }
