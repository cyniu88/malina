...
Upgrade-Insecure-Requests: 1
    ...
    std::string msgGET2 = R"(HTTP GET /api/gate_open.html HTTP/1.1
Host: cyniu88.no-ip.pl:48833
User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:102.0) Gecko/20100101 Firefox/102.0
Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8
Accept-Language: pl,en-US;q=0.7,en;q=0.3
...
{
    EXPECT_STREQ( Http::getUrl(msgGET2).c_str(), "/api/gate_open.html" );
    EXPECT_STREQ( Http::getUrl(msgGET).c_str(), "/api/gate_open.html" );
    EXPECT_STREQ( Http::getUrl(msgPOST).c_str(), "/iDom/log" );
}

TEST_F(http_class_fixture, getQuery)
{
   auto ov = Http::getQuery(msgGET);
