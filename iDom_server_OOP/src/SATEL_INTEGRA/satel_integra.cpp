...
std::string SATEL_INTEGRA::getIntegraInfo()
{
    ...
    rec << "Wersja: " << m_message[4] << "." << m_message[5] << m_message[6] << " z dnia: " << m_message[7] << m_message[8] << m_message[9] << m_message[10] << "-" << m_message[11] << m_message[12] << '-' << m_message[13] << m_message[14] << std::endl;
...
int SATEL_INTEGRA::recvIntegra()
{
    ...
    if (m_message[0] not_eq INTEGRA_ENUM::HEADER_MSG or m_message[1] not_eq INTEGRA_ENUM::HEADER_MSG or m_message[size - 1] not_eq INTEGRA_ENUM::END or m_message[size - 2] not_eq INTEGRA_ENUM::HEADER_MSG)
    {
    ...
    {
        log_file_mutex.mutex_lock();
        log_file_cout << CRITICAL << "Satel Integra: receive bad CRC :(" << std::endl
                      << "crc & 0xFF          = " << (crc & 0xFF) << std::endl
