...
{
    auto i = jj.at("response").get<nlohmann::json>();
    std::cout <<"\n\n data all" << i.at(0).dump(4) << std::endl;
    auto size = i.size();
    ...
    for (auto j : i){
        ...
        bearingAver.push_back(j.at("relativeTo").at("bearing").get<int>());
    }
    std::cout << std::endl;
    ...
    std::cout << "kierunek ostatniego uderzenia pieruna: "<< bearingAver.average()<<std::endl;
    std::cout << std::endl;
}
